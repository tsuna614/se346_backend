...
const postController = {
  ...
  reportPost: async (req, res, next) => {
    try {
      ...
      //Check if user has already reported post
      const report = await Report.findOne({ userId: userId, postId: postId });
      //If yes, ignore  ,still return 200. If no, create report
      if (!report) {
        await Report.create({
          userId: userId,
          postId: postId,
